...
def main(exp_config) -> None:
    ...
    make_dot(yhat, params=dict(list(net.named_parameters()))).render("densenet", format="png")

    input_names = ['Input image']
    output_names = ['Predicted class']
    torch.onnx.export(net, inputs, 'densenet.onnx', input_names=input_names,
                      output_names=output_names)

    wandb.init(
        # Set the project where this run will be logged
        project="diploma-audio-data",
